...
- 2026-08-21：D-12 修复交付（`ApiExceptionHandler` 新增 `NoResourceFoundException`→404 NOT_FOUND、`HttpRequestMethodNotSupportedException`→405 METHOD_NOT_ALLOWED）后，最新交付可用性黑盒测试（`LatestDeliveryAvailabilityTest`，7 项）执行：LD-01 未知端点 404、LD-02 方法不支持 405、LD-03 损坏段隔离后库可用（SF-3）、LD-04 删除快照后同内容重放真正写入（SF-1）、LD-05 混合批导入（SF-2）、LD-06 跨表并发导入（SF-4）、LD-07 常规回归——全部通过；黑盒全量 31 项 BUILD SUCCESS；`avUnknownEndpoint404` 解除 `@Disabled` 启用并通过 → D-12 关闭（已验证）。
