...
- **可空列（nullable）**：建表时每列可选可空（主键列强制非空）。存储使用 **null 位图**（每行 1 bit）：可空列编码"有效值序列"（跳过 null 行，delta/gorilla 压缩不因占位退化）+ 位图标记 null 位置；全非空列不写位图（零空间开销）。CSV/二进制导入时空字段 → null（可空列）或拒绝（非空列）。查询（分页/全量/单点历史）还原 null。
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/api/createTable` | 建表：schema（列名/类型/**nullable 可空**）、主键列（强制非空）、保留期、压缩等级 |
...
- 非空列出现空值 → 400 拒绝（导入前校验，含主键列）；格式版本 v2（可空列位图），旧 v1 数据文件不兼容读取（需重新导入）；
